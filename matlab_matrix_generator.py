...
def matGen(matrix):
    ...
    if codeM == "B = A(:,:)":
        ...
        return str(codeM)
# Function to toggle a button's state and update the matrix
...
# Create a nested list in memory initialized to 0
matrix = [[0 for j in range(num_cols)] for i in range(num_rows)]

#m = matGen(matrix)

# Create a new window with increased height and width
window = tk.Tk()
window.geometry("500x250")

# Create a matrix of buttons initialized to 0
buttons = []
...
# Add a label widget on the far right side of the window to display the nested list
...
# Add a label widget on the far right side of the window to display the label
label = tk.Label(window, text="Kod do Matlaba", font=("Arial Bold", 12))
